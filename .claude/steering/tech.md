...
| ADR-023 | Scheduler memakai tick per menit yang menyelaraskan diri, BUKAN pustaka cron — meski CLAUDE.md menyebut node-cron | ADR-019 menyimpan JENDELA, bukan cron. Pustaka cron berarti menurunkan ekspresi per jendela, menyerahkannya ke parser, lalu memercayai perjalanan bolak-baliknya — padahal pertanyaan tiap menit hanya "apakah menit ini salah satu waktu tembak jendela ini?", yang dijawab jendelanya sendiri lewat `firesAt`. Nol dependency baru | Tick menyelaraskan ke puncak menit supaya tidak melenceng. MENGASUMSIKAN SATU INSTANCE API: dua replika akan menembak dobel — perlu advisory lock Postgres sebelum scale-out. `firesAt` dan `framesPerDay` WAJIB sepakat, kalau tidak batas paket menjaga jumlah capture yang berbeda dari yang benar-benar diambil |
