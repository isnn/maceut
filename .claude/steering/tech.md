...
| ADR-021 | Paket berbayar DIBERIKAN staf, tidak bisa dipilih sendiri; `POST /me/onboarding` tidak lagi menerima `plan`, dan `PATCH /me/plan` menolak kenaikan paket dengan 403 `UPGRADE_NOT_SELF_SERVE` | Tanpa billing, sebuah pemilih paket bukan penjualan — itu formulir yang membagikan batas Premium ke siapa pun yang membaca halaman harga. Dua jalur self-serve terbuka lebar: langkah onboarding dan tombol ganti paket di Profil. Deployment yang bisa dijangkau siapa saja bisa dijangkau siapa saja | Upgrade lewat `PATCH /internal/users/{id}/plan`, sehingga ada catatan siapa memberi apa. PENURUNAN paket sengaja tetap self-serve: melepas kapasitas tidak merugikan bisnis, dan memaksa orang membuka tiket untuk berhemat itu tidak masuk akal — penurunan tetap menjalankan ADR-020. Saat billing ada, jalur upgrade jadi payment intent + webhook, bukan melonggarkan cek ini |
| ADR-022 | Kolom `organisation` pada user DIHAPUS (migrasi 0006), bukan disembunyikan | Label teks bebas yang dikumpulkan saat daftar dan tidak pernah dibaca oleh logika mana pun. Di formulir pendaftaran ia duduk bersebelahan dengan nama lengkap, seolah bergabung ke sebuah organisasi adalah bagian dari membuat akun — persis paradigma yang sudah dibuang ADR-020. Kolom yang tidak dibaca siapa pun adalah cara sebuah schema mengumpulkan field yang tujuannya tak bisa direkonstruksi setahun kemudian | DESTRUKTIF: nama organisasi yang sudah masuk ikut hilang, tidak ada tempat lain yang menyimpannya. Direktori internal kehilangan satu dimensi pencarian — diganti kolom jumlah zona, yang lebih berguna untuk operator. Kalau kontak pelanggan ternyata perlu, tempatnya tabel customer-record yang menyatakan itu, bukan warisan paradigma lama |
| ADR-023 | Waktu tembak berikutnya disimpan di `schedules.next_fire_at`; scheduler MENG-KLAIM baris jatuh tempo lewat satu `UPDATE ... FOR UPDATE SKIP LOCKED`, lalu mem-publish ke RabbitMQ. Tidak ada pustaka cron | ADR-019 menyimpan JENDELA, bukan cron — pertanyaan tiap bangun hanya "apa yang jatuh tempo?", yang bisa dijawab satu kolom. Menaruh jam di memori berarti downtime menelan tembakan tanpa jejak; menaruhnya di broker (pesan tertunda) memindahkan sumber kebenaran keluar dari database, di mana satu pesan hilang mematikan jadwal selamanya tanpa ada yang bisa mendeteksi | KLAIM ITU SENDIRI ADALAH LOCK: dua instance API kini aman, batasan single-instance hilang. Scheduler TIDUR sampai jatuh tempo berikutnya (maks 60 dtk), bukan polling tiap menit. `nextFireAfter` DITURUNKAN dengan memanggil `firesAt` — bukan rumus kedua — karena rumus kedua persis cara `framesPerDay` melenceng. Tembakan yang lewat >5 menit TIDAK diambil; dicatat sebagai `missed` |
| ADR-024 | Tembakan yang terlewat saat sistem mati DICATAT sebagai baris `missed`, tidak dijalankan belakangan | Nilai frame 07:00 adalah bahwa ia DARI 07:00. Mengambilnya 11:40 adalah jawaban lain yang menyesatkan, dan ikut menghabiskan kuota harian untuk sesuatu yang tidak diminta. Tapi diam saja lebih buruk lagi: jendela tetap aktif, dashboard tetap memprediksi, zonanya sekadar berlubang | Satu baris per outage per jendela, bukan satu per tembakan — gap 5 jam pada jendela 15 menit adalah 20 baris identik. `missed` tidak menghitung ke batas harian (tidak ada capture) maupun ke kegagalan (tidak ada yang dicoba). Batas toleransi 5 menit |
