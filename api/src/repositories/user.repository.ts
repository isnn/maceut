import { eq, sql, desc, asc, and, ilike, or, type SQL } from 'drizzle-orm'
...
export async function listWithPlans(opts: ListUsersOptions): Promise<{ rows: UserWithPlan[]; total: number }> {
  ...
  if (opts.role) filters.push(eq(user.role, opts.role))
...
export async function countByRole(role: PlatformRole): Promise<number> {
  const rows = await db.select({ count: sql<number>`count(*)::int` }).from(user).where(eq(user.role, role))
