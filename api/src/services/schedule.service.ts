...
async function scheduleInWorkspace(workspaceId: string, id: string): Promise<ScheduleRecord> {
  const row = await scheduleRepo.findById(id)
  if (!row || row.status === 'deleted') throw new NotFoundError('Jadwal')
  if (row.workspaceId !== workspaceId) throw new ForbiddenError('Jadwal ini bukan milik workspace Anda.')
  return row
}
...
/**
 * BR-006 read forwards: a window must not commit the workspace to more frames per day
...
async function requireDailyBudget(
  ...
  const existing = await scheduleRepo.findByWorkspaceId(workspaceId)
...
export async function listSchedules(workspaceId: string): Promise<PublicSchedule[]> {
  return (await scheduleRepo.findByWorkspaceId(workspaceId)).map(toPublic)
}

export async function listForZone(workspaceId: string, zoneId: string): Promise<PublicSchedule[]> {
  const zone = await zoneRepo.findById(zoneId)
  if (!zone || zone.workspaceId !== workspaceId) throw new NotFoundError('Zona')
...
export async function createSchedule(
  ...
  // The zone must belong to this workspace — otherwise a schedule could be attached
  ...
  if ((await scheduleRepo.countActive(workspaceId)) >= limit) {
    throw new ScheduleLimitExceededError(limit)
  }

  await requireDailyBudget(workspaceId, plan, input)
...
export async function updateSchedule(
  ...
  const existing = await scheduleInWorkspace(workspaceId, id)
  ...
  if (willBeActive && existing.status !== 'active') {
    const limit = PLAN_LIMITS[plan].schedulesLimit
    if ((await scheduleRepo.countActive(workspaceId, id)) >= limit) {
      throw new ScheduleLimitExceededError(limit)
    }
  }

  if (willBeActive && (shapeChanged || existing.status !== 'active')) {
    await requireDailyBudget(workspaceId, plan, next, id)
  }
...
export async function deleteSchedule(workspaceId: string, id: string): Promise<void> {
  await scheduleInWorkspace(workspaceId, id)
  await scheduleRepo.softDelete(id)
}

/** Total frames per day across every active window — the board's budget readout. */
export async function totalFramesPerDay(workspaceId: string): Promise<number> {
  const rows = await scheduleRepo.findByWorkspaceId(workspaceId)
  return rows
