      ...
      <div className="flex flex-wrap items-end justify-between gap-md">
        <div>
          <p className="text-label text-text-secondary">
            Paket {PLAN_LABEL[plan]} · maksimal {limits.schedulesLimit} jendela aktif · Asia/Jakarta GMT+7
          </p>
          <h1 className="text-page-title font-bold text-text-primary mt-xs">Jendela koleksi</h1>
        </div>
        <Button className="h-11 px-lg" onClick={() => setAddOpen(true)} disabled={zones.length === 0}>
          Tambah jendela
        </Button>
      </div>
      ...
      <div className="grid grid-cols-1 laptop:grid-cols-[1fr_320px] gap-xl items-start">
        <div className="bg-card border border-border rounded-lg p-lg space-y-lg">
          {windows === null ? (
            ...
            <EmptyState
              title="Belum ada zona untuk dijadwalkan"
              description="Buat zona dulu, lalu tentukan kapan zona itu mengumpulkan data."
              action={
                <Link
                  href="/zones/new"
            ...
            <>
              ...
              <div className="grid grid-cols-[160px_1fr] gap-md items-center">
                <span className="text-micro uppercase tracking-wide text-text-muted">Zona</span>
                <div className="flex">
                  {HOURS.map((hour) => (
                    ...
                    <li key={zone.id} className="grid grid-cols-[160px_1fr] gap-md items-center">
                      <div className="min-w-0">
                        ...
                        <p className="text-micro text-text-muted mt-xs">
                          {zone.status === 'paused'
                            ? 'Dijeda — lanjutkan untuk mengumpulkan'
                            : `${zoneWindows.length} jendela · ${zoneWindows.reduce((s, w) => s + framesPerDay(w), 0)} frame/hari`}
                        </p>
                      </div>
        ...
        <div className="space-y-lg">
          <Card className="p-lg space-y-md">
            <div>
              <p className="text-label text-text-secondary">Frame per hari</p>
              ...
              {framesTotal > limits.capturesLimit && (
                <p className="text-caption text-warning-text mt-sm">
                  Melebihi batas harian paket — capture di atas batas akan dilewati (BR-006).
                </p>
              )}
            </div>
            <div className="border-t border-divider pt-md">
              <p className="text-label text-text-secondary">Jendela aktif</p>
              <p className="text-body font-semibold text-text-primary tabular-nums">
                {activeCount} / {limits.schedulesLimit}
              </p>
            </div>
            <div className="border-t border-divider pt-md">
              <p className="text-label text-text-secondary">Retensi</p>
              <p className="text-body font-semibold text-text-primary">{limits.historyLabel}</p>
              {plan !== 'premium' && <p className="text-caption text-text-muted mt-xs">Tanpa batas di Premium</p>}
            </div>
          </Card>

          <Card className="p-lg">
            <p className="text-label text-text-secondary">Koleksi berikutnya</p>
            <p className="text-page-title font-bold text-text-primary tabular-nums mt-xs">10:00</p>
            <p className="text-caption text-text-muted mt-xs">dalam 21 menit · {activeCount} jendela</p>
          </Card>
        </div>
      </div>
      ...
      <Dialog.Portal>
        ...
        <Dialog.Popup className="fixed z-50 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-[30rem] max-h-[90vh] overflow-y-auto bg-card border border-border rounded-lg p-xl shadow-elevation-3">
          <Dialog.Title className="text-section-title text-text-primary">
            {editing ? 'Ubah jendela' : 'Tambah jendela koleksi'}
          </Dialog.Title>
          <Dialog.Description className="text-caption text-text-secondary mt-xs mb-lg">
            {editing
              ? `${editing.capturedFrames} frame terkumpul sejak jendela ini dibuat.`
              : `Paket ${PLAN_LABEL[plan]} mengizinkan ${PLAN_LIMITS[plan].schedulesLimit} jendela aktif.`}
          </Dialog.Description>
          ...
          <div className="space-y-lg">
            ...
            <div className="space-y-xs">
              <FormLabel htmlFor="window-zone">Zona</FormLabel>
            ...
            <div className="space-y-xs">
              <FormLabel htmlFor="window-label">Nama jendela</FormLabel>
              <Input id="window-label" placeholder="Puncak pagi" value={label} onChange={(e) => setLabel(e.target.value)} />
            </div>

            <div className="grid grid-cols-2 gap-md">
              <div className="space-y-xs">
                <FormLabel htmlFor="window-start">Mulai</FormLabel>
                <Input id="window-start" type="time" value={start} onChange={(e) => setStart(e.target.value)} />
              </div>
              <div className="space-y-xs">
                <FormLabel htmlFor="window-end">Selesai</FormLabel>
            ...
            <p className="text-caption text-text-secondary bg-canvas-secondary rounded-md p-md">
              Menambah <span className="font-semibold text-text-primary tabular-nums">{frames} frame/hari</span> ke
              anggaran capture Anda.
            </p>
          </div>
          ...
          <div className="flex items-center justify-between gap-sm mt-xl">
            ...
            <div className="flex gap-sm">
              ...
              <Button className="h-10 px-lg" onClick={save} disabled={saving || !zoneId}>
                {saving ? 'Menyimpan...' : editing ? 'Simpan perubahan' : 'Tambah jendela'}
              </Button>
            </div>
          </div>
          ...
          {confirmRemove && (
            <div className="mt-lg border border-danger-text/30 bg-danger-bg rounded-md p-md">
              <p className="text-body text-danger-text font-medium">Hapus jendela ini?</p>
              <p className="text-caption text-danger-text/80 mt-xs">Frame yang sudah ter-capture tetap disimpan.</p>
              <div className="flex gap-sm mt-md">
                <Button variant="destructive" className="h-9 px-md" onClick={remove}>
                  Hapus
                </Button>
                <Button variant="secondary" className="h-9 px-md" onClick={() => setConfirmRemove(false)}>
                  Batal
                </Button>
              </div>
            </div>
