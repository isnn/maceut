...
async function seedIfEmpty(plan: Plan) {
  ...
  if (collecting[0]) {
    seeded.push(
      makeWindow(collecting[0].id, 'Morning peak', '06:00', '11:00', 'hourly', WEEKDAYS, 412),
      makeWindow(collecting[0].id, 'Evening peak', '16:00', '21:00', 'hourly', WEEKDAYS, 388)
    )
  }
  if (collecting[1]) {
    seeded.push(makeWindow(collecting[1].id, 'All day', '07:00', '15:00', 'hourly', WEEKDAYS, 264))
  }
  writeWindows(seeded)
}
...
export async function createWindow(input: CreateWindowInput, plan: Plan): Promise<CaptureWindow> {
  ...
  if (activeCount >= PLAN_LIMITS[plan].schedulesLimit) {
    ...
    throw new ApiError({
      ...
      message: `You have reached your plan's limit of ${PLAN_LIMITS[plan].schedulesLimit} active windows.`,
    })
  }
  if (input.interval === '15min' && plan !== 'premium') {
    await delay(null)
    throw new ApiError({ code: 'FORBIDDEN', message: 'The 15-minute interval is Premium only.' })
  }
  const created: CaptureWindow = {
    id: generateId(),
    zoneId: input.zoneId,
    label: input.label || 'New window',
    start: input.start,
    end: input.end,
...
export async function updateWindow(id: string, patch: Partial<CaptureWindow>): Promise<CaptureWindow> {
  ...
  if (!found) {
    await delay(null)
    throw new ApiError({ code: 'NOT_FOUND', message: 'Window not found.' })
  }
